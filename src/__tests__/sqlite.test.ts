...
describe('KiroDatabase: cleanupTestAndStaleAccounts', () => {
  test('removes test@example.com rows', async () => {
    await db.upsertAccount(makeAccount({ id: 'real', email: 'real@example.com' }))
    await db.upsertAccount(makeAccount({ id: 'test-1', email: 'test@example.com' }))
    const removed = await db.cleanupTestAndStaleAccounts()
    expect(removed).toBe(1)
    expect(db.getAccounts().map((a) => a.id)).toEqual(['real'])
  })

  test('removes placeholder-*@awsapps.local rows', async () => {
    await db.upsertAccount(makeAccount({ id: 'real', email: 'real@example.com' }))
    await db.upsertAccount(makeAccount({ id: 'ph-1', email: 'placeholder-abc@awsapps.local' }))
    const removed = await db.cleanupTestAndStaleAccounts()
    expect(removed).toBe(1)
    expect(db.getAccounts().map((a) => a.id)).toEqual(['real'])
  })

  test('removes permanently-unhealthy rows past recovery window', async () => {
    await db.upsertAccount(makeAccount({ id: 'real', email: 'real@example.com' }))
    await db.upsertAccount(
      makeAccount({
        id: 'stale',
        ...
        isHealthy: false,
        unhealthyReason: 'Account Suspended',
        recoveryTime: 0
      })
    )
    const removed = await db.cleanupTestAndStaleAccounts(0)
    expect(removed).toBe(1)
    expect(db.getAccounts().map((a) => a.id)).toEqual(['real'])
  })

  test('keeps healthy accounts and recent perm-fail rows', async () => {
    await db.upsertAccount(makeAccount({ id: 'healthy', email: 'healthy@example.com' }))
    await db.upsertAccount(
      makeAccount({
        id: 'recent-fail',
        ...
        isHealthy: false,
        unhealthyReason: 'Account Suspended',
        recoveryTime: Date.now() + 86400000
      })
    )
    const removed = await db.cleanupTestAndStaleAccounts()
    ...
    expect(
      db
        ...
        .map((a) => a.id)
        .sort()
    ).toEqual(['healthy', 'recent-fail'])
  })

  test('returns 0 when there is nothing to clean', async () => {
    await db.upsertAccount(makeAccount({ id: 'only', email: 'only@example.com' }))
    expect(await db.cleanupTestAndStaleAccounts()).toBe(0)
  })
})
